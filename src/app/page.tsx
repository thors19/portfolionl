...
export default function Dashboard() {
  const { stocks, crypto, metals, isLoading, lastUpdated, refreshPrices } = usePortfolio();

  const totalEtfs    = stocks.filter((s) => s.assetType === "etf").reduce((sum, p) => sum + (p.marktwaarde ?? p.degiroWaardeEur ?? 0), 0);
  const totalAandelen = stocks.filter((s) => s.assetType !== "etf").reduce((sum, p) => sum + (p.marktwaarde ?? p.degiroWaardeEur ?? 0), 0);
  const totalStocks  = totalEtfs + totalAandelen;
  const totalCrypto  = crypto.reduce((s, p) => s + (p.marktwaarde ?? 0), 0);
  const totalMetals  = metals.reduce((s, m) => s + (m.marktwaarde ?? 0), 0);
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Totaaloverzicht van je vermogen</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subValue={`${stocks.length} positie${stocks.length !== 1 ? "s" : ""} · ${stocks.filter((s) => s.assetType === "etf").length} ETF`}
        ...
        <StatCard
          ...
          subValue={`${crypto.length} munt${crypto.length !== 1 ? "en" : ""}`}
        ...
        <StatCard
          ...
          subValue={`${metals.length} positie${metals.length !== 1 ? "s" : ""}`}
